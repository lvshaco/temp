...
local batchid = 20160804   ---批次id
local gift_treasure = "10013:1"  --礼包组
local code_level = 100
local code_time = "2016/02/29-2016/04/15"
local code_type = 3
	...
	for k,v in ipairs(tpcode) do
		while index <= v.count  do
			...
			local result = conn:execute(sfmt("insert into x_exchange set exchange = '%s', batchid = %d, exchange_type = %d,gift_treasure = '%s',use_level = %d,effective_time = '%s'"
											,code,v.id,v.type,v.gift_treasure,v.Level,v.Time))
